...
import { getHTCKS, layoutToggleLoading } from "actions";
...
class Page extends React.Component {
  ...
  onAdjust = async (value) => {
    ...
    delete param.key;
    // console.log(param);
    await dispatch(adjustTEMUsing(jwt, param));
  };
  ...
  onReject = async ({ pdndung: lyDo }) => {
    ...
    await dispatch(
      rejectTEMUsing(jwt, { id, lyDo, trangThai: trangThai === 0 ? 2 : 4 })
    );
  };
  ...
  onApprove = async () => {
    ...
    try {
      ...
      await dispatch(
        rejectTEMUsing(jwt, { id, trangThai: trangThai === 0 ? 1 : 3 })
      );
    } catch { }
  };
